...
import { siteFor, useSite, type SiteIdentity } from "@/lib/site-identity";
import { getRequestOrigin } from "@/lib/request-origin.functions";

function buildFaqs(SITE: SiteIdentity): Array<{ category: string; q: string; a: string }> {
  return [
  ...
  { category: "Returns & Warranty", q: "How do I start a return?", a: `Email ${SITE.supportEmail} with your order number. We'll send a prepaid return label within 24 hours.` },
  ...
  { category: "Returns & Warranty", q: "What if my item arrives damaged?", a: `We're sorry! Email a photo to ${SITE.supportEmail} within 7 days of delivery and we'll send a free replacement immediately.` },
...
export const Route = createFileRoute("/faq")({
  ...
  head: ({ loaderData }) => {
    ...
    const FAQS = buildFaqs(SITE);
    const { meta, links } = buildOg({
      origin,
      path: "/faq",
      title: `FAQ — ${SITE.name} Help Center`,
      description: `Shipping times, returns, warranty coverage, product care, and account help — every common ${SITE.name} question answered in one place.`,
      imageAlt: `${SITE.name} FAQ and help center`,
      type: "website",
    });
    return {
      meta,
    ...
    <BreezyLayout>
      <section className="bg-[#F2EDE3]">
        <div className="max-w-4xl mx-auto px-6 py-20 text-center">
          ...
          <p className="text-[#5A554C]">Can't find what you need? <a href={`mailto:${SITE.supportEmail}`} className="text-[#5A7A55] underline">Email our team</a> — we usually reply within a few hours.</p>
        </div>
      </section>
