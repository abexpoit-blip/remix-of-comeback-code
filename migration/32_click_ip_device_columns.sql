-- Fix: clicks.ip_address and clicks.device were never populated by the batch RPC.
-- Analytics/quality audits (unique IPs, device split) read ip_address/device, so they
-- always showed 0. This mirrors ip -> ip_address and extracts device from signals.
-- Everything else is identical to migration 25.
...
AS $$
  ...
  inserted_clicks AS (
    ...
    link_id, ip, ip_address, device, country, ua, is_bot, bot_reason, routed_to,
    utm_source, utm_medium, utm_campaign, utm_term, utm_content,
    referer_host, bot_score, signals, challenge_passed
    )
    SELECT
      link_id, ip, ip, NULLIF(signals->>'device', ''), country, ua, is_bot, bot_reason, routed_to,
      utm_source, utm_medium, utm_campaign, utm_term, utm_content,
      referer_host, bot_score, signals, challenge_passed
    FROM events
...
-- Backfill the last 7 days so audits work immediately.
...
UPDATE public.clicks
SET device = NULLIF(signals->>'device', '')
WHERE device IS NULL AND signals ? 'device'
  AND created_at > now() - interval '7 days';

NOTIFY pgrst, 'reload schema';

SELECT 'clicks ip_address + device logging fixed' AS status;
